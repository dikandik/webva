...
# Konfigurasi
AXWAY_SERVER="10.197.19.105:8075"
USERNAME="apiadmin"
PASSWORD="P@ssw0rdBD!"
BASE_URL="https://$AXWAY_SERVER/api/portal/v1.4"
DISCOVERY_URL="https://$AXWAY_SERVER/api/portal/v1.3/discovery"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_CSV="api_from_org_${TIMESTAMP}.csv"
OUTPUT_XLS="api_from_org_${TIMESTAMP}.xls"
...
# 1. Get organizations
echo "Mengambil daftar Organizations..."
curl -k -s "${HEADERS[@]}" "$BASE_URL/organizations" -o temp_apps.json
ORG_IDS=$(jq -r '.[].id' temp_apps.json)
...
# Prepare CSV content
CSV_CONTENT="Organizations Name,API Access\n"

# 3. Process each organizations
for ORG_ID in $ORG_IDS; do
    # Get organizations details
    ORG_DETAIL=$(curl -k -s "${HEADERS[@]}" "$BASE_URL/organizations/$ORG_ID")
    ORG_NAME=$(echo "$ORG_DETAIL" | jq -r '.name')
    
    # Get subscriptions
    curl -k -s "${HEADERS[@]}" "$BASE_URL/organizations/$ORG_ID/apis" -o temp_subscriptions.json
    SUBSCRIPTION_COUNT=$(jq length temp_subscriptions.json)
    
    # Tampilkan output simple
    echo "========================================="
    echo "Organizations: $ORG_NAME"
    echo "API Access:"
    ...
    CLEAN_ORG_NAME=$(echo "$ORG_NAME" | sed 's/,/;/g')
    CSV_CONTENT="$CSV_CONTENT\"$CLEAN_ORG_NAME\",\"$SUBSCRIPTIONS_CSV\"\n"
